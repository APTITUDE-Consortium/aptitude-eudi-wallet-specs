...
>
> The APTITUDE UC7 material provides a concrete European Disability Card example with
> identity attributes, portrait, card serial number, expiry date, QR code, and an indication
> of entitlement to an accompanying person or additional support. This Rulebook uses that
> source-defined subset as its starting point and adds a proposed privacy-preserving
> entitlement model for SEDIT-X. Proposed claims, identifiers, vocabularies, status
> infrastructure and trust endpoints require confirmation through the APTITUDE WP2/WP4
> governance process before production use.
...
3. **Minimum identity disclosure:** name, birth date, portrait and card number SHALL
   be requested only where needed for fraud prevention, visual inspection or legal rules.
4. **No inferred diagnosis:** a verifier SHALL NOT infer a diagnosis from an entitlement.
5. **Context-specific requests:** a ferry operator, airport, hotel or university SHALL
   request only claims needed for the specific service.
6. **User control:** presentation SHALL require informed User approval.
7. **No unnecessary retention:** verifiers SHOULD retain only the decision and minimum
   operational reference.
8. **Cross-domain reuse:** one credential MAY support multiple service domains where
   the entitlement semantics are accepted by each relevant service policy.
9. **Fallback:** a User who cannot or does not present the credential SHALL retain access
...
|---------------------|----------------|---------------|-------------------|
| `attestation_id` | Unique identifier of this Accessibility / Disability Attestation. | string | `ada_01JZ4A8M2V7Q9K6P3R5T` |
...
| `portrait` | Portrait image of the holder. Included only where the scheme requires visual inspection or anti-fraud checks. | binary or URI/reference | `data:image/jpeg;base64,...` |
...
| `qr_code` | Scheme-defined QR code or protected machine-readable card representation. | binary or string | `EDCQR:...` |
...
| `issuing_authority_reference` | Registry or authority reference used to verify issuer authorisation. Mandatory where the trust framework requires it. | URI or string | `https://authority.example/disability-card-issuers/IT` |
...
| `status_reference` | Reference used for status or revocation checking. | URI or structured value | `https://status.example/atl/2026-07/42#812` |
...
| `portrait` | `picture` | string or binary reference | OIDC-related claim; profile to be confirmed | MUST |
...
| `attestation_id` | `attestation_id` | string | Credential identifier | MUST NOT |
...
| `qr_code` | `qr_code` | string or object | Scheme QR representation | MUST |
...
| `issuing_authority_reference` | `issuing_authority_reference` | string | Authority/registry reference | MUST NOT |
...
```json
{
  ...
  "attestation_id": "ada_01JZ4A8M2V7Q9K6P3R5T",
...
* unsupported free-text descriptions of disability; or
...
### 4.5 Identity attributes and portrait

The UC7 source-defined European Disability Card includes family name, given name,
birth date, portrait and card serial number.

These attributes MAY be present in the issued credential, but a verifier SHALL request
them only where needed.

Examples:

* a ticket portal generally SHOULD request the entitlement and scheme validity, not portrait;
* a staff member visually inspecting a card MAY request portrait and minimal name data;
* a ferry or airport verifier MAY request a subject-binding reference rather than full name;
* a remote service SHOULD avoid portrait unless a justified anti-fraud process requires it.
...
4. the credential may support transport, hospitality and academic services;
5. user consent is required for presentation;
6. verifiers request only context-relevant entitlements;
7. special-category data receives enhanced protection;
8. status and revocation are supported;
9. credential copies are not routinely retained; and
10. manual fallback remains available.
...
* handling of temporary entitlements;
* use of portrait and QR code; and
