...
> credential identifier, QR binary profile, trust-list endpoint or status-list endpoint.
> The pilot model for this draft is an **embedded encrypted biometric payload** (no remote
> biometric retrieval). Values and requirements explicitly identified below as **pilot profile**
...
entitlement were previously verified and bound. The token carries, embedded within the
credential itself, an encrypted biometric payload (protected portrait together with journey
context) used solely for one-to-one (1:1) verification against a live facial capture.

The attestation SHALL NOT contain any reference, URI or identifier that resolves to
biometric data stored outside the credential.
...
- **Protected biometric payload** means the encrypted, self-contained structure carried in
the credential that contains the portrait (or biometric template) used exclusively as the
reference side of a 1:1 biometric comparison, together with the journey context needed by
an authorised checkpoint. It is never a pointer to biometric data stored elsewhere.
...
only the data needed to verify the airport journey context and to perform a one-off 1:1
biometric comparison against the embedded encrypted biometric payload.
...
6. **No central biometric database:** live captures, matching artefacts and the issued
  biometric payload SHALL NOT be persistently stored by the Attestation Provider, airport or
  airline systems after the purpose that required them has been completed.
7. **Embedded biometric only:** the credential SHALL embed the encrypted biometric payload
  and SHALL NOT rely on remote biometric retrieval.
...
| `biometric_binding_type`      | Type of biometric material embedded in the credential.                                                           | string enum      | `protected_portrait`                       |
...
- `protected_portrait`; or
- `protected_template`.

An implementation SHALL select one binding model in its deployment profile. A relying party
SHALL reject a credential containing an unsupported binding type. The credential SHALL
always embed the biometric material; values that identify an external biometric store are
not permitted.
...
| `biometric_quality`           | Issuer-generated quality result for the embedded biometric material, without exposing matching thresholds.                  | string enum   | `sufficient`                |
| `enrolment_method`            | Method used to obtain the biometric material embedded in the credential.                                                    | string enum   | `pid_portrait`              |
...
| `biometric_binding_type`      | `biometric_binding_type`      | string              | Embedded biometric representation type              | MUST NOT        |
| `protected_biometric_payload` | `protected_biometric_payload` | string              | Base64url-encoded encrypted biometric payload; SHALL NOT be an external reference | MUST |
...
portrait or template together with the journey context required by authorised checkpoints.
It SHALL be selectively disclosable as a single claim. An Attestation Provider SHALL
structure the credential so that a verifier cannot independently disclose nested elements
that are not required for the authorised comparison. The Wallet Unit SHALL hold only the
encrypted payload and SHALL NOT retain a plaintext portrait after issuance. The claim
SHALL NOT contain a URI, URN or other identifier that resolves to biometric data stored
...
```json
{
  ...
  "biometric_binding_type": "protected_portrait",
  "protected_biometric_payload": "eyJlbmMiOiJBMjU2R0NNIn0..3v9X...",
...
- the embedded biometric payload will be used only for a one-off 1:1 comparison;
...
The QR presentation SHALL be self-contained. It SHALL carry the encrypted
`protected_biometric_payload` and SHALL NOT depend on retrieval of biometric data from a
remote store at verification time.

The encrypted payload SHALL contain at least:

- the protected portrait or protected template required for 1:1 comparison; and
- the journey context needed by an authorised checkpoint (including flight identification
  and authorised checkpoint scope, as defined by the deployment profile).

The final binary/QR encoding profile is not fully specified at the current version. The APTITUDE pilot profile SHALL define:

- maximum payload size and QR error-correction level;
- cryptographic protection, encryption format and reader-validation rules;
- verifier decryption-key distribution and rotation;
...
7. expose the biometric material only in encrypted form within
...
6. obtain or derive the biometric material according to one of the approved methods, encrypt
  it together with the required journey context into `protected_biometric_payload`, and
  embed that payload in the credential;
7. delete the source portrait, enrolment captures and any plaintext biometric material from
  issuer processing systems after issuance, and SHALL NOT retain a server-side copy of the
  biometric payload;
8. display the credential type, issuer, purpose, intended airport use, attributes and biometric
  processing notice to the User; and
9. obtain explicit User consent before issuance.
...
9. where biometric-enhanced mode is used, decrypt `protected_biometric_payload` and perform
  only 1:1 comparison against the biometric material contained in the presented token;
10. obtain biometric material exclusively by decrypting the presented credential and SHALL NOT
  retrieve biometric data from any remote system for this purpose;
11. discard live captures, derived templates and decrypted reference data from processing memory
  immediately after the matching decision; and
12. return only the minimum operational outcome needed by the touchpoint.
...
- the live capture SHALL be compared only against the biometric material decrypted from
`protected_biometric_payload` in the presented token;
...
6. issuer authorisation scope, including authorised airports and credential types;
7. the entities authorised to hold decryption keys for `protected_biometric_payload`, and
  how those keys are distributed, rotated and revoked; and
8. Wallet Unit and verifier behaviour when trust information cannot be retrieved.
...
- biometric payload encryption format and verifier key-management profile;
