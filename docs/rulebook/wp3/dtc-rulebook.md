...
In addition, the framework may also support ICAO DTC Type 1 where it is based on an LDS (Logical Data Structure) signed by the official passport issuing authority. In such cases, the DTC-VC is encapsulated within an attestation stored in the EUDI Wallet, ensuring that it remains cryptographically linked to the passport (DTC-PC physical component) and provides sufficient assurance for border control use cases.
