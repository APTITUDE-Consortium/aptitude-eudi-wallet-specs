...
- use of **<artifacts:Wallet Instance Attestation (WIA)>** as client attestation material at the PAR and Token endpoints where Authorisation Code Flow is used;
...
- support for **<artifacts:Wallet Unit Attestation (WUA)>** and `proofs` structures at the Credential Endpoint for device-bound issuance;
...
For Authorisation Code Flow, this profile assumes the Wallet interacts with the <components:Authorisation Server> using PAR, receives an authorisation code, exchanges it at the Token Endpoint, and then calls the Credential Endpoint. Under ETSI, <artifacts:Wallet Instance Attestation (WIA)|WIA> is carried at both the PAR and Token endpoints, together with <processes:Proof of Possession|proof-of-possession> of the key referenced in the <artifacts:Wallet Instance Attestation (WIA)|WIA> `cnf` claim.

For device-bound issuance, ETSI requires the Credential Request to carry a `proofs` parameter containing either `jwt` or `attestation`. In the `jwt` mechanism, the Wallet proves possession of a key to be bound to the credential and carries the <artifacts:Wallet Unit Attestation (WUA)|WUA> in the `key_attestation` protected header parameter. In the `attestation` mechanism, the Wallet carries the <artifacts:Wallet Unit Attestation (WUA)|WUA> directly without separate <processes:Proof of Possession|proof-of-possession> of each attested key. The issuer's processing rules differ accordingly and will be profiled in the normative sections below.
...
For **device-bound issuance**, the Wallet sends a Credential Request containing a `proofs` structure as profiled by this RFC. ETSI TS 119 472-3 requires use of **<artifacts:Wallet Unit Attestation (WUA)>** at the Credential Endpoint, either carried in the protected header of `proofs.jwt[*]` or directly in `proofs.attestation[*]`, depending on the mechanism used.
...
1. The Wallet retrieves Issuer metadata, including at minimum:
    ...
    - supported proof methods and related <data-elements:Nonce|nonce> requirements;
...
3. Where this profile requires **<artifacts:Wallet Instance Attestation (WIA)>** for the Authorisation Code Flow path, the Wallet includes:
   - the <artifacts:Wallet Instance Attestation (WIA)|WIA> at the PAR request; and
   - <processes:Proof of Possession|proof-of-possession> of the key referenced in the <artifacts:Wallet Instance Attestation (WIA)|WIA> `cnf` claim, according to the profiled mechanism.
...
2. Where this profile requires **<artifacts:Wallet Instance Attestation (WIA)>** for the Authorisation Code Flow path, the Wallet includes:
   - the <artifacts:Wallet Instance Attestation (WIA)|WIA> at the Token Endpoint; and
   - <processes:Proof of Possession|proof-of-possession> of the key referenced in the <artifacts:Wallet Instance Attestation (WIA)|WIA> `cnf` claim.
...
1. Before calling the Credential Endpoint, the Wallet obtains a fresh <data-elements:Nonce|nonce> from the Issuer's **<data-elements:Nonce> Endpoint** where required by the selected proof method or issuance profile.
...
4. Where the selected proof method is based on `proofs.jwt`:
   ...
   - the Wallet carries the **<artifacts:Wallet Unit Attestation (WUA)>** in the protected header parameter profiled for key attestation.
5. Where the selected proof method is based on `proofs.attestation`:
   - the Wallet carries the **<artifacts:Wallet Unit Attestation (WUA)>** directly in the attestation structure; and
...
6. The <roles:Attestation Provider (AP)|Attestation Provider> validates:
   ...
   - the <data-elements:Nonce|nonce> and proof material;
...
2. The Wallet follows the same Authorisation Code Flow defined in Section 6.1 for:
   ...
   - credential request;
   - storage.
3. Where this profile requires **<artifacts:Wallet Instance Attestation (WIA)|WIA>** for this path, the Wallet sends <artifacts:Wallet Instance Attestation (WIA)|WIA> at the PAR and Token endpoints as described in Section 6.1.
...
2. The request includes:
   ...
   - <artifacts:Wallet Unit Attestation (WUA)|Wallet Unit Attestation> where required by the selected proof method or profile.
...
3. Support <artifacts:Wallet Instance Attestation (WIA)|Wallet Instance Attestation> handling at the PAR and Token endpoints where required by this profile.
...
7. Require <artifacts:Wallet Instance Attestation (WIA)> at the PAR endpoint where this profile applies ETSI-aligned wallet attestation.
8. Require <processes:Proof of Possession|proof-of-possession> of the public key referenced in the <artifacts:Wallet Instance Attestation (WIA)|WIA> `cnf` claim.
...
11. Verify that the <processes:Proof of Possession|proof-of-possession> validates under the public key referenced in the <artifacts:Wallet Instance Attestation (WIA)|WIA> `cnf` claim.
...
5. Include <artifacts:Wallet Instance Attestation (WIA)|WIA> at the PAR endpoint where this profile requires it.
6. Include <processes:Proof of Possession|proof-of-possession> of the public key referenced in the <artifacts:Wallet Instance Attestation (WIA)|WIA> `cnf` claim.
...
1. Send a Token Request containing:
   ...
   - `redirect_uri`;
   - `code_verifier`.
2. Include <artifacts:Wallet Instance Attestation (WIA)|WIA> at the Token Endpoint where this profile applies ETSI-aligned wallet attestation.
3. Include <processes:Proof of Possession|proof-of-possession> of the public key referenced in the <artifacts:Wallet Instance Attestation (WIA)|WIA> `cnf` claim.
...
4. Where <artifacts:Wallet Instance Attestation (WIA)|WIA> is required, validate:
   ...
   - the <processes:Proof of Possession|proof-of-possession> under the public key referenced in the <artifacts:Wallet Instance Attestation (WIA)|WIA> `cnf` claim.
...
7. Validate the <data-elements:Nonce|nonce> and proof material according to the selected proof method.
8. Validate <artifacts:Wallet Unit Attestation (WUA)> according to the selected proof method.
9. Return either:
...
4. Obtain and use a fresh <data-elements:Nonce|nonce> from the Issuer's <data-elements:Nonce> Endpoint where required by the selected proof method.
...
2. Include a valid <data-elements:Nonce|nonce> claim in the JWT body, obtained from the Issuer's <data-elements:Nonce> Endpoint.
3. Include the `key_attestation` protected header parameter.
4. Carry the <artifacts:Wallet Unit Attestation (WUA)> in that `key_attestation` parameter.
5. Ensure that the <artifacts:Wallet Unit Attestation (WUA)|WUA> contains one or more attested public keys owned by the <components:Wallet Unit>.
6. Sign the JWT using the private key corresponding to the first public key in the `attested_keys` claim of the <artifacts:Wallet Unit Attestation (WUA)|WUA>.

Issuers **SHALL**:

1. Verify that the <artifacts:Wallet Unit Attestation (WUA)|WUA> signature in `key_attestation` validates under the <roles:Wallet Provider (WP)> public key from the applicable trusted list.
2. Verify that the JWT signature validates under the first public key in the `attested_keys` array of the <artifacts:Wallet Unit Attestation (WUA)|WUA>.
3. Verify that the JWT contains a valid <data-elements:Nonce|nonce> from the Issuer's <data-elements:Nonce> Endpoint.
4. Generate as many credentials as there are attested public keys in the <artifacts:Wallet Unit Attestation (WUA)|WUA>, where the applicable profile requires multi-key issuance.
...
2. Include a valid <data-elements:Nonce|nonce> claim in the JWT body, obtained from the Issuer's <data-elements:Nonce> Endpoint.
3. Carry the <artifacts:Wallet Unit Attestation (WUA)> directly as the single attestation element.

Issuers **SHALL**:

1. Verify that the <artifacts:Wallet Unit Attestation (WUA)|WUA> signature validates under the <roles:Wallet Provider (WP)> public key from the applicable trusted list.
2. Generate as many credentials as there are attested public keys in the <artifacts:Wallet Unit Attestation (WUA)|WUA>, where the applicable profile requires multi-key issuance.
...
2. Distinguish clearly between <data-elements:Nonce|nonce> failure, attestation failure, proof failure, and policy failure.
...
If the Issuer requires a fresh <data-elements:Nonce|nonce> for proof generation, the Issuer **SHALL** expose and document the relevant mechanism in metadata.
...
| `VCI-CHECK-03A` | <artifacts:Wallet Instance Attestation (WIA)\|Wallet Instance Attestation> handling | For authorization-code issuance where the profile requires wallet attestation, the wallet sends <artifacts:Wallet Instance Attestation (WIA)\|WIA> at the PAR and Token endpoints, proves possession of the key referenced in the <artifacts:Wallet Instance Attestation (WIA)\|WIA> `cnf` claim, and the issuer validates those inputs. | ETSI-aligned PAR and token endpoint attestation handling; ARF wallet attestation baseline |
...
| `VCI-CHECK-06A` | <artifacts:Wallet Unit Attestation (WUA)\|Wallet Unit Attestation> handling | The credential request carries <artifacts:Wallet Unit Attestation (WUA)\|WUA> in the correct location for the selected proof method, the issuer validates the <artifacts:Wallet Unit Attestation (WUA)\|WUA>, and the issued credential is bound according to the attested key material. | ETSI-aligned credential endpoint attestation handling; ARF key-attestation and holder-binding baseline |
